...
def extractPreMiRName(miRNA_coordinate, miRNA_database):
	miRNamePreNameDic = {}
		...
		with open(miRNA_coordinate, 'r') as inf:
			for line in inf:
				if line[0] != '#':
					content = line.strip().split('\t')
					if content[2] == 'miRNA':
						name = content[-1].split('ID=')[1].split(';')[0].strip()
						stemLoopName = '_'.join(name.split('_')[:-1])+'_pre'
						miRNamePreNameDic.update({name:stemLoopName})
