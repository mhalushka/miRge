...
The two functions of miRge2.0 are:\n\
   annotate      Annotate the reads from miRNA sequencing data\n\
                 Type "miRge2.0.py annotate -h" to show the help message of this funtion\n\
   predict       Detect novel miRNAs from miRNA sequencing data\n\
                 Type "miRge2.0.py predict -h" to show the help message of this funtion\n'
	...
	parser_1.add_argument('-sp', required=True, dest='species', metavar='<string required>', help="The species can be human, mouse, fruitfly, nematode, rat and zebrafish (novle miRNA detection is confined in human and mouse)")
	#parser_1.add_argument('-pi', required=True, dest='indexPath', metavar='<dir required>', help="The path to bowtie's index files, (index files of miRNA, hairpin, mrna, snoRNA and trna are compulsory, spike-ins is necessory when turning it on and index files of genome is necessory when launching novle miRNA detection)")
	...
	#parser_1.add_argument('-sp', required=True, dest='species', metavar='<string required>', help="The species where the sequencing data are from. (novle miRNA detection is confined in human)")
	...
	parser_2.add_argument('-sp', required=True, dest='species', metavar='<string required>', help="The species can be human, mouse, fruitfly, nematode, rat and zebrafish (novle miRNA detection is confined in human and mouse)")
	#parser_2.add_argument('-pi', required=True, dest='indexPath', metavar='<dir required>', help="The path to bowtie's index files, (index files of miRNA, hairpin, mrna, snoRNA and trna are compulsory, and index files of genome is necessory when launching novle miRNA detection)")
	...
	#parser_2.add_argument('-sp', required=True, dest='species', metavar='<string required>', help="The species where the sequencing data are from. (novle miRNA detection is confined in human)")
	...
	parser_2.add_argument('-ws',  default='none', dest ='wideSampleListFile', metavar='<file>', help='the file containing the overall samples to analysis for novel miRNA prediction (GodenStandardSampleList.txt)')
	parser_2.add_argument('-minl', default='16', dest ='minLength', metavar='<int>', help='the minimum length of the reatined reads (default: 16)')
	parser_2.add_argument('-maxl', default='25', dest='maxLength', metavar='<int>', help='the maximum length of the reatined reads (default: 25)')
	parser_2.add_argument('-cc', default='2', dest='countCutoff', metavar='<int>', help='the maximum read count of the reatined reads (default: 2)')
	parser_2.add_argument('-ml', default='3', dest='mapping_loc', metavar='<int>', help='the maximum number of mapping loci for the retained reads (default: 3)')
	parser_2.add_argument('-sl', default='25', dest='seedLength', metavar='<int>', help='the seed length when invoking Bowtie (default: 25)')
	parser_2.add_argument('-olc', default='14', dest='overlapLenCutoff', metavar='<int>', help='the length of overlapped seqence when joining reads into longer sequences based on the coordinate on the genome (default: 14)')
	parser_2.add_argument('-clc', default='30', dest='clusterSeqLenCutoff', metavar='<int>', help='the maximum length of the clustered sequences (default: 30)')
