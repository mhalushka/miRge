...
3) If the required python pacakages of the specific version can't be installed by pip or can't be imported by python, make sure the installed python is complied by 4-byte Unicode so that pip can install UCS4 wheels (supporting cp27mu not cp27m). Type python and enter following commands "import sys" "print  sys.maxunicode". If output is 1114111 then it is UCS4 otherwise if output is 65535 then it is UCS2.<br />
